...
static void execute_instruction(std::vector<GridState>& grid, const Instruction& instruction)
{
    for (int y = instruction.start.y; y <= instruction.end.y; ++y) {
        for (int x = instruction.start.x; x <= instruction.end.x; ++x) {
            ...
            case InstructionType::on:
                grid[index] = GridState::on;
                break;
            case InstructionType::off:
                grid[index] = GridState::off;
                break;
            case InstructionType::toggle:
                grid[index] = grid[index] == GridState::on ? GridState::off : GridState::on;
                break;
            }
        }
    }
...
static int solve(const std::string& data)
{
    ...
    grid.resize(grid_size * grid_size, GridState::off);
    for (int pos = 0; pos < data.length(); ++pos) {
        Instruction instruction = parse_instruction(data, pos);
        execute_instruction(grid, instruction);
    }
    const int on_count = std::ranges::fold_left(grid, 0, [&](const int count, const GridState state) {
        return state == GridState::on ? count + 1 : count;
    });
    return on_count;
}
